...
func TestTransport_Frames_DecodesScriptedEventsAsProtoJSON(t *testing.T) {
	...
	for f := range tr.Frames() {
		...
		if len(f.Raw) == 0 {
			t.Error("expected Raw to contain deterministic protobuf wire bytes")
		}
		if bytes.Equal(f.Data, f.Raw) {
			t.Errorf("expected binary Raw to differ from protojson Data, got %q", f.Raw)
		}
		envelope := new(agentstreampb.StreamEvent)
		if err := protobuf.Unmarshal(f.Raw, envelope); err != nil {
			t.Fatalf("Frame.Raw did not decode as the full StreamEvent envelope: %v", err)
		}
		...
		frames = append(frames, struct {
			...
			data map[string]any
			raw  []byte
		}{f.Name, decoded, append([]byte(nil), f.Raw...)})
	}
...
func TestTransport_Frames_OneofRawPreservesEnvelope(t *testing.T) {
	srv, err := mockgrpc.New([]*agentstreampb.StreamEvent{
		{Payload: &agentstreampb.StreamEvent_AgentContent{AgentContent: &agentstreampb.AgentContent{
			AgentId: "agent_a", Content: "hello", Sequence: 9,
		}}},
	})
	if err != nil {
		t.Fatalf("mockgrpc.New: %v", err)
	...
	frames := make([]transport.Frame, 0, 1)
	...
	frame := frames[0]
	if frame.Name != "agent_content" {
		t.Fatalf("expected oneof name agent_content, got %q", frame.Name)
	}
	if bytes.Equal(frame.Data, frame.Raw) {
		t.Fatal("expected oneof Data protojson and Raw protobuf wire bytes to differ")
	}
	envelope := new(agentstreampb.StreamEvent)
	if err := protobuf.Unmarshal(frame.Raw, envelope); err != nil {
		t.Fatalf("expected Raw to decode as StreamEvent envelope: %v", err)
	}
	content := envelope.GetAgentContent()
	if content == nil || content.GetAgentId() != "agent_a" || content.GetSequence() != 9 {
		t.Fatalf("expected Raw envelope to contain agent_content, got %+v", content)
	}
}
