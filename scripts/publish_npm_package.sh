#!/usr/bin/env bash
# Publish the npm wrapper package for an existing agent-stream-dbg release.
# Same pattern as Obedience-Corp/festival scripts/publish_npm_package.sh.

set -euo pipefail

PACKAGE_DIR="${NPM_PACKAGE_DIR:-npm}"
PACKAGE_NAME="${NPM_PACKAGE_NAME:-@obedience-corp/agent-stream-dbg}"
...
if [ "$publish_status" -ne 0 ]; then
    if printf '%s\n' "$publish_output" | grep -q 'EOTP'; then
        echo "::error::npm publish requires OTP. Re-run interactively, configure trusted publishing, or use a token with bypass 2FA." >&2
    fi
    exit "$publish_status"
fi
